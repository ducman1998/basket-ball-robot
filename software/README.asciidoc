...
=== Teleoperation Node
The `teleop` node allows for manual control of the robot using keyboard inputs. It captures key presses and translates them into velocity commands, which are then published to the `/cmd_vel` topic. This node is useful for testing and debugging the robot's movement capabilities.

==== Usage
To run the `teleop` node, use the following command:
[source,shell]
----
# start the teleop node
ros2 run teleop start_teleop
----
Make sure your terminal is focused when using the teleop node, as it captures keyboard inputs directly.
...
=== Lauch Game Logic Node (Robot Finds A Ball)
To launch the game logic node along with the other nodes, you can use the following command:
[source,shell]
----
# launch game logic node along with other nodes
ros2 launch basket_robot_bringup robot_find_a_ball.launch.py
----
This command will start the Game Logic Node only. Other functional nodes (mainboard_controller, odometry, image_processor) need to be launched separately.
